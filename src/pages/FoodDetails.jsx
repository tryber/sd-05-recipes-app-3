...
function FoodDetails(props) {
  ...
  const recipeGetter = (ingredient) => {
    const items = Object.keys(ingredient).filter((keysIngridients) => keysIngridients.includes('strIngredient')).filter(value => ingredient[value] !== '');
    const quantities = Object.keys(ingredient).filter((qtIngridients) => qtIngridients.includes('strMeasure')).filter(value => ingredient[value] !== '');
  ...
  return (idRecipe) ? (
    <div>
      ...
      <div className="card-recipe">
        <img alt={strMeal} className="card-recipe-image" src={strMealThumb} data-testid="recipe-photo" />
        <div className="card-recipe-body">
          <h3 className="card-recipe-name" data-testid="recipe-title">{strMeal}</h3>
          <p data-testid="recipe-category"></p>
          <img src={favorite ? blackHeartIcon : whiteHeartIcon} alt="whiteHeart" data-testid="favorite-btn" onClick={() => handleFavorite() } />
